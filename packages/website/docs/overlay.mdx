...
import { Overlay, Popup } from 'ali-react-overlays';
...
浮层组件默认会使用 [React Portal](https://reactjs.org/docs/portals.html) 并配合 absolute 或 fix 定位方式将内容渲染到 `document.body` 中。如果指定了 `props.portalContainer`，组件会将内容渲染到对应的容器中。ali-react-overlays 也提供了 [`OverlayBehaviorContext`](app-context) 用于设置浮层默认挂载节点。
...
`Overlay.animations` 提供了 Overlay 默认支持的动画效果，例如 `Overlay.animations.fadeIn`。完整列表可以在 Overlay 源码中进行查看。ali-react-overlays 中的动画效果与 [animate.css](https://animate.style/) 相兼容，你也可以**引入 animate.css**，然后使用 animate.css 提供的动画效果。例如：
