...
import { Dialog } from 'ali-react-overlays';
import { PlacementDiagram } from '../src/examples/position.stories';
import * as stories from '../src/examples/dialog.stories';
...
对话框提供以下 9 种定位方式，通过 `placement` 可以指定对话框在容器内的位置，默认为 `"center"`。
...
<div className="aro-dialog">
  <div className="aro-dialog-header">{title}</div>
  <div className="aro-dialog-body">{children}</div>
...
对于一些非必选/不紧急的对话框，你可以设置 `canCloseByEsc={true}` 和 `canCloseByOutSideClick={true}` 来使用非模态对话框。
...
使用 structure 可以完全定制对话框的结构，此时 title 和 footer 属性无效。
