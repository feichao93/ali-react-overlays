...
const defaultRenderTarget: PopupProps['renderTarget'] = (htmlProps, { target, targetStyle, targetTagName }) => {
  return React.createElement(
    targetTagName,
    {
      ...htmlProps,
      style: targetStyle,
      // aro-popup-target will set display=inline-block
      className: 'aro-popup-target',
    },
    target,
  );
};
...
const adjustArrowStyleModifier: Popper.Modifier<'adjustArrowStyle', {}> = {
  ...
  fn({ state }) {
    ...
    if (state.placement.startsWith('top')) {
      ...
      state.elements.popper.style.setProperty('--aro-popup-arrow-position', `${state.modifiersData.arrow.x}px bottom`);
    }
    if (state.placement.startsWith('right')) {
      state.styles.arrow.left = '0';
      state.elements.popper.style.setProperty('--aro-popup-arrow-position', `left ${state.modifiersData.arrow.y}px`);
    }
    if (state.placement.startsWith('bottom')) {
      state.styles.arrow.top = '0';
      state.elements.popper.style.setProperty('--aro-popup-arrow-position', `${state.modifiersData.arrow.x}px top`);
    }
    if (state.placement.startsWith('left')) {
      state.styles.arrow.right = '0';
      state.elements.popper.style.setProperty('--aro-popup-arrow-position', `right ${state.modifiersData.arrow.y}px`);
    }
  },
};
...
export class Popup extends React.Component<PopupProps, PopupState> {
  ...
  private reopenPopper() {
    ...
    target.dataset.aroPopupOpen = 'true';
  ...
  private clearTargetDataset = () => {
    const target = this.targetRef.current as HTMLElement;
    delete target?.dataset.aroPopupOpen;
  };
  ...
  render() {
    ...
    const overlayLifecycles = _.pick(this.props, [
      'beforeOpen',
      'onOpen',
      'afterOpen',
      'beforeClose',
    ...
    const arrow = hasArrow && <div className="aro-popup-arrow" data-popper-arrow="true" />;
        ...
        <Overlay
          ...
          renderChildren={({ ref }) => (
            <div
              ...
              className={cx('aro-popup-content', className)}
              style={style}
