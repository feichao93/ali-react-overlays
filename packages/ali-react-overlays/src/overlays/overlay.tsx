...
import { DOCUMENT_BODY, OverlayBehaviorContext, OverlayBehaviorContextType } from '../context';
import { domUtils } from '../dom-utils';
import { ANIMATE_ANIMATED, ANIMATE_PREFIX, Disposable, startAnimate } from '../utils/animate-utils';
...
export interface IOverlayAnimationProps {
   ...
   * @category 浮层动画
   * */
  animation?: false | { in: string; out: string };
}
...
export class Overlay extends React.Component<OverlayProps, OverlayState> {
  ...
  static getManager(portalContainer: HTMLElement | typeof DOCUMENT_BODY) {
    ...
    if (portalContainer == null) {
      console.warn('[ali-react-overlays] Overlay.getManager(portalContainer) 调用参数 portalContainer 为 null');
      return;
    }
  ...
  private async doOpenOverlay() {
    ...
    inner.classList.add('aro-overlay-inner');
  ...
  private resolveExitAnimation(instruction: IOverlayAnimationProps): null | string {
    const { animation: animationProp } = this.props;
    if (animationProp === false || instruction?.animation === false) {
      return null;
    }
  ...
  componentDidUpdate(prevProps: Readonly<OverlayProps>, prevState: Readonly<OverlayState>) {
    if (prevProps.disableScroll !== this.props.disableScroll) {
      console.warn('[ali-react-overlays] 浮层组件 props.disableScroll 不支持动态变化');
    }
    if (prevProps.attachOverlayManager !== this.props.attachOverlayManager) {
      console.warn('[ali-react-overlays] 浮层组件 props.attachOverlayManager 不支持动态变化');
    }
  ...
  public isInsideClick = (clickNode: Node) => {
    // 背景层总是认为是外部
    if ((clickNode as Element).classList?.contains?.('aro-overlay-backdrop')) {
      return false;
    }
  ...
  render() {
    ...
    let content = (
      <div
        className={cx('aro-overlay-wrapper', { exiting, detached: !attachOverlayManager }, className)}
      ...
      >
        {hasBackdrop && (
          <div
            className={cx(
              'aro-overlay-backdrop',
              this.props.backdropClassName,
              ANIMATE_ANIMATED,
              exiting ? `${ANIMATE_PREFIX}${animations.fadeOut}` : `${ANIMATE_PREFIX}${animations.fadeIn}`,
            )}
            style={{
    ...
    if (usePortal) {
      content = createPortal(content, portalContainer, overlayKey);
    }
