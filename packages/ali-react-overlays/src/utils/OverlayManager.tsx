...
export class OverlayManager {
  static ARO_OVERFLOW_HIDDEN_CLS = 'aro-overflow-hidden';
  static ARO_OVERFLOW_FORCE_HIDDEN_CLS = 'aro-overflow-force-hidden';
  ...
  add(overlay: Overlay) {
    requestAnimationFrame(() => {
      ...
      this.portalContainer.dataset.aroOverlayCount = String(this.stack.length);

      if (overlay.props.disableScroll === true) {
        this.disableScrollCount += 1;
        if (this.disableScrollCount === 1) {
          this.portalContainer.classList.add(OverlayManager.ARO_OVERFLOW_HIDDEN_CLS);
        }
      } else if (overlay.props.disableScroll === 'force') {
        this.forceDisableScrollCount += 1;
        if (this.forceDisableScrollCount === 1) {
          this.portalContainer.classList.add(OverlayManager.ARO_OVERFLOW_FORCE_HIDDEN_CLS);
        }
      }
  ...
  delete(overlay: Overlay) {
    ...
    if (overlay.props.disableScroll === true) {
      ...
      if (this.disableScrollCount === 0) {
        this.portalContainer.classList.remove(OverlayManager.ARO_OVERFLOW_HIDDEN_CLS);
      }
    } else if (overlay.props.disableScroll === 'force') {
      this.forceDisableScrollCount -= 1;
      if (this.forceDisableScrollCount === 0) {
        this.portalContainer.classList.remove(OverlayManager.ARO_OVERFLOW_FORCE_HIDDEN_CLS);
      }
    }
